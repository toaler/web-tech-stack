		...
		if (enabled) {
			try (CuratorFramework curatorFramework = CuratorFrameworkFactory
				...
				try (ServiceDiscovery<String> sd = ServiceDiscoveryBuilder.<String>builder(String.class)
						.basePath("/service-discovery").client(curatorFramework).thisInstance(serviceInstance)
						.build()) {
						...
						public void run() {
							if (sd != null) {
								try {
									...
									System.err.println("Failed to close service discover due to " + e.getMessage()
											+ " cause=" + e.getCause() + " stack="
											+ Joiner.on(",").join(e.getStackTrace()));
								}
							}
						}
					});
